...
public class DispatchingFunctionsIntegrationTests {
    ...
    void packAndLabelOrder() {
        // FunctionCatalog에서 합성 함수를 가져온다.
        Function<OrderAcceptedMessage, Flux<OrderDispatchedMessage>> packAndLabel =
                catalog.lookup(Function.class, "pack|label");
        long orderId = 121;

        StepVerifier.create(packAndLabel.apply(new OrderAcceptedMessage(orderId)))      // 함수에 대한 입력인 OrderAcceptedMessage 정의
                .expectNextMatches(dispatchedOrder ->                                   // 함수의 출력이 OrderDispatchedMessage 객체인지 확인한다.
